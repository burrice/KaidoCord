...
src/
  ...
  plugins/
    ...
    base64Auto/               # KAIDO plugin: detects Base64 strings in messages (≥20 chars, >50% printable), adds inline "B64 ▼/▲" decode button; max 3 per message
    bulkDelete/               # KAIDO plugin: mass-delete own messages via channel-context and user-context menus
    channelExport/            # KAIDO plugin: exports channel history to JSON or CSV via "📤 Exportar Canal" channel context submenu; 4 items (JSON-100, JSON-500, CSV-100, CSV-500); 300ms batch delay; patches "channel-context" only
    crashHandler/             # Upstream plugin; KAIDO rebrand: maybePromptToUpdate string is mixed EN/PT-BR ("Uh oh, Discord crashed... but há uma atualização...")
    customStatusPlus/         # KAIDO plugin: rotating custom status with emoji support and configurable interval
    cveLinker/                # KAIDO plugin: patches parseToAST to linkify CVE-XXXX-NNNN patterns → NVD URLs (nvd.nist.gov/vuln/detail/)
    gameActivityToggle/       # Upstream plugin; partial rebrand: location option label "Kaidocord Toolbox"
    ghostMode/                # KAIDO plugin: BOOLEAN setting "enabled" calls setLocalPresence({status:"invisible"}) on enable, restores "online" on stop()
    hashScan/                 # KAIDO plugin: detects MD5/SHA1/SHA256/SHA512 hashes in messages; HashBadge links directly to VirusTotal search
    ipGeo/                    # KAIDO plugin: detects first IPv4 in message content; "🌍 Lookup IP:" message context menu → IPModal (ip-api.com, fields: country/region/city/isp/org/as)
    ...
    messageScheduler/         # KAIDO plugin: /schedule <30s|5m|2h> <msg>, /scheduled (list), /unschedule <id>; in-memory array cleared on stop()
    messageTemplates/         # KAIDO plugin: DataStore key "KaidoTemplates"; commands /t /tsave /tlist /tdel; names stored lowercase
    networkMonitor/           # KAIDO plugin: patches window.WebSocket (gateway URLs only); MAX_EVENTS=200 (newest-first); /netmon command opens MonitorModal; WebSocket NOT restored on stop()
    phishGuard/               # KAIDO plugin: 8 SUSPICIOUS_PATTERNS + typosquatting heuristic; "⚠️ Possível Phishing Detectado" red-bordered message decoration
    profileStats/             # KAIDO plugin: account-age badge (BadgePosition.END) via getBadges() array shape; inline SVG CalendarIcon
    quickPoll/                # KAIDO plugin: /poll ("|"-separated, up to 10 options, 1️⃣–🔟 reactions) and /yesno (✅/❌)
    secureNote/               # KAIDO plugin: AES-256-GCM encrypted notes; PBKDF2 key derivation (salt "kaidocord-salt-v1", 100k iterations); DataStore key "KaidoSecureNotes"; /notes command
    ...
    tokenWatcher/             # KAIDO plugin: pre-send listener intercepts Discord tokens (TOKEN_RE); Alert with "Redigir e enviar" / "Cancelar envio" / "Enviar assim mesmo"
    typingPrivacy/            # KAIDO plugin: patches startTyping with "if(true)return;" early exit; no settings, no start()/stop()
    urlExpander/              # KAIDO plugin: detects 10 known URL shorteners; ExpandButton fetches unshorten.me/s/ proxy; shows expanded URL in green (#00e676)
...
- `settings.tsx`: `vencord_main` sidebar title IS "Kaidocord" (rebranded); Updater `panelTitle: "Vencord Updater"` (pending rebrand); plugin option descriptions `"Where to put the Vencord settings section"` and `"Also copy Vencord info (Vencord, Electron, Chromium)"` (pending rebrand)
- `supportHelper.tsx`: `renderContributorDmWarningCard` and `CHANNEL_SELECT` alert bodies reference "Vencord plugin developers", Vencord guild/channel URLs, and "externally updated Vencord" / "custom build of Vencord"; `renderMessageAccessory` detects `"/vencord-debug"` and `"/vencord-plugins"` message strings and renders `"Run /vencord-debug"` / `"Run /vencord-plugins"` button labels; `CHANNEL_SELECT` checks `VENCORD_GUILD_ID` for role trust; only the debug info object key `"Kaidocord"` and slash command names are rebranded; debug info GitHub URL uses `https://github.com/teamkaido/kaidocord/commit/` — inconsistent with `package.json` repo URL `github.com/burrice/KaidoCord`
- `offline.ts`: `downloadSettingsBackup` output filename still uses `vencord-settings-backup-` prefix (only the file-picker filter label "Kaidocord Settings Backup" is rebranded); `toastSuccess` body is EN (`"Settings successfully imported. Restart to apply changes!"`) and `toastFailure` body is EN — both pending PT-BR rebrand
...
- UI strings facing users in PT-BR (e.g., update notifications, settings tab labels); code and docs in EN — exceptions: cloud pull success notification body in `Vencord.ts`, `serverSpy` context menu item label, and `offline.ts` toastSuccess/toastFailure bodies are still EN (pending rebrand); `crashHandler` maybePromptToUpdate string is mixed EN/PT-BR (starts "Uh oh, Discord crashed... but"); cloud auth check notification in `Vencord.ts` is already PT-BR
...
- BulkDelete rate-limit pattern: 350ms delay between individual deletes (`RestAPI.del`) hardcoded in loop (`setTimeout(r, 350)`) — `deleteDelay` SLIDER setting (default 350, markers [200,350,500,750,1000]) is defined but NOT applied to the loop (bug/oversight); 1500ms backoff on error; fetches messages in batches of 100, filters by `me.id`, stops when `deleted >= limit` or messages exhausted; patches both `"channel-context"` and `"user-context"` menus; Alert offers "Deletar 100" / "Deletar 500" options
...
- CrashHandler rebrand: `maybePromptToUpdate` call string is mixed EN/PT-BR (`"Uh oh, Discord crashed... but há uma atualização do Kaidocord disponível que pode resolver! Deseja atualizar agora?"`); crash recovery notification bodies remain upstream EN
...
- MessageDecoration plugin pattern (`addMessageDecoration` / `removeMessageDecoration`): decoration key prefix is `"kaido-"` (e.g. `"kaido-base64"`, `"kaido-hashscan"`, `"kaido-phishguard"`, `"kaido-urlexpander"`); component receives `{ message }` prop and returns `null` when nothing detected — avoids rendering empty nodes
- Base64Auto detection gate: `isLikelyBase64()` requires `length % 4 === 0` OR string ends with `"="`, then `atob()` must succeed and decoded bytes must be >50% printable (charCode 32–126); `detectBase64()` caps at 3 matches per message and deduplicates via `Set`
- ChannelExport batch fetch: 300ms `setTimeout` delay between 100-message API pages; exported JSON shape is `{id, author, content, timestamp, attachments[]}`; CSV escapes `"` by doubling; both filenames include `Date.now()` suffix for uniqueness; 4 submenu items under "📤 Exportar Canal": JSON-100, JSON-500, CSV-100, CSV-500 — all direct children of the submenu; only patches `"channel-context"` (not `"user-context"`)
- CVELinker patch target: `"parseToAST"` — wraps the call site via `$self.preprocessCVE()` injection; link target is `https://nvd.nist.gov/vuln/detail/{CVE}` (NVD, not Mitre directly)
- GhostMode presence: uses `findByProps("setLocalPresence", "updateAsync")` at call time (not module load); `stop()` always restores `status: "online"` regardless of initial state
- HashScan regex order: `HASH_PATTERNS` array is defined [MD5(32), SHA1(40), SHA256(64), SHA512(128)]; each pattern uses independent `matchAll` per pattern; `Set` deduplication on the hash string prevents the same value from being tagged twice regardless of pattern order — ordering concern is moot in practice
- IPGeo API: `http://ip-api.com/json/{ip}` (HTTP, not HTTPS) with query `?fields=status,country,regionName,city,isp,org,as,query`; only the first IPv4 address found in `message.content` is offered for lookup
- MessageScheduler delay spec: `parseDelay()` accepts `"Ns"` (×1000ms), `"Nm"` (×60000ms), `"Nh"` (×3600000ms) — any other format returns `null` and shows FAILURE toast; `stop()` calls `clearTimeout` on all pending entries then sets `scheduled.length = 0`
- MessageTemplates storage: `get`/`set` from `@api/DataStore`; key `"KaidoTemplates"`; all names lowercased before store/lookup — `/t Foo` and `/t foo` are equivalent
- NetworkMonitor WebSocket patch: `patchedWS` boolean guards against double-patching; only intercepts connections where `url.includes("gateway")`; the patched `window.WebSocket` is NOT restored on `stop()` — requires Discord reload to revert
- PhishGuard typosquatting heuristic: `domain.includes(legit.replace(".", ""))` AND `domain.length <= legit.length + 3` AND `domain !== legit`; false positives are possible — pattern is intentionally simple
- QuickPoll option cap: max 10 options (`.slice(1).slice(0, 10)`); emoji array `REACTIONS` is `["1️⃣"..."🔟"]`; `/yesno` always outputs ✅/❌ with no options array
- SecureNote crypto: IV is 12 bytes (`crypto.getRandomValues(new Uint8Array(12))`); stored as `btoa(iv + ciphertext)` concatenated; `readNote()` uses `window.prompt()` for password — blocking native dialog; wrong password causes `crypto.subtle.decrypt` to throw, caught and shown as Toasts.FAILURE
- TokenWatcher token regex: anchored on first char `[MN]` (covers user and bot tokens); `redactTokens()` keeps the header segment (`parts[0]`) intact to help identify the account; "Enviar assim mesmo" secondary button sends original unreacted content
- TypingPrivacy patch: injects `if(true)return;` at the open brace of `startTyping($1){` — the `if(true)` form avoids linter "unreachable code" errors on the remainder; plugin has no `start()`/`stop()` methods and no settings
- URLExpander proxy: uses `https://unshorten.me/s/{encodeURIComponent(url)}` — response body is the final URL as plain text; on network error sets expanded state to `"(erro ao expandir)"`; URLs longer than 80 chars are truncated with `…` in the display anchor
<!-- END AUTO-MANAGED -->
